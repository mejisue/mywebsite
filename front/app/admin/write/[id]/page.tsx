...
import { ChangeEvent, KeyboardEvent, use, useEffect, useRef, useState } from 'react';
...
import { getPost, updatePost, uploadImage } from '@/lib/api/posts';
...
export default function AdminEditPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { mutate: editPost, isPending } = useMutation({
        mutationFn: () => updatePost(id, { title, content, tags }),
        onSuccess: () => {
            router.push(`/post/${id}`);
        },
        onError: (error) => {
