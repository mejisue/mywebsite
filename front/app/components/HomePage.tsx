...
const PROJECT_ITEMS: (ProjectItem & {
  ...
  image?: string;
})[] = [
    {
      ...
      mobileSize: '5rem',
      detail: {
        description: '지금 보고 계신 이 포트폴리오 사이트입니다. GSAP 기반 룸 씬 인트로, Next.js App Router + Spring Boot 풀스택 구조, 어드민 블로그 에디터, Docker Compose 프로덕션 배포까지 실제로 운영 중인 서비스로 직접 설계·구현했습니다.',
        motivation: '\'나열형 포트폴리오\' 대신 \'작동하는 풀스택 서비스\'를 보여주고 싶었습니다. 직접 만든 어드민으로 글을 쓰고, 직접 설계한 인프라로 운영되는 사이트 자체가 가장 솔직한 기술 증명이라고 판단했습니다.',
        techStack: ['Next.js', 'TypeScript', 'Spring Boot', 'MySQL', 'Redis', 'Docker', 'AWS EC2', 'GSAP'],
        demoGif: '/mywebsite-demo.webm',
        designPoints: [
          {
            title: 'GSAP 기반 룸 씬 애니메이션',
            problem: '단순 스크롤·페이드 인트로는 포트폴리오로서 차별화 포인트가 없음',
            solution: 'GSAP Timeline으로 문 열기 → 룸 씬 전환을 단계별 제어. SVG를 레이어별로 분리해 각 요소를 독립 타임라인으로 순차 애니메이션 구성.',
            result: '방문자가 진입 시 고유한 인터랙션 경험 제공',
            video: '/mywebsite-door.webm',
          },
          {
            title: 'Next.js App Router + Spring Boot 풀스택 인증',
            problem: 'RSC 환경에서 쿠키 기반 인증 상태를 서버·클라이언트 양쪽에서 일관되게 유지해야 하는 문제',
            solution: 'Spring Boot가 HttpOnly 쿠키로 JWT 발급, Next.js middleware에서 쿠키 검증 후 라우팅 제어. 클라이언트는 쿠키를 직접 읽지 않아 XSS 방어.',
            result: 'XSS 방어 + 서버·클라이언트 인증 상태 일관성 확보',
          },
          {
            title: 'Docker Compose 멀티 서비스 프로덕션 배포',
            problem: 'Next.js FE·Spring Boot BE·MySQL·Redis를 단일 서버에서 안정적으로 운영',
            solution: 'Docker Compose로 서비스별 컨테이너 분리 및 healthcheck 의존성 설정, Nginx 리버스 프록시로 단일 도메인에서 FE·BE 라우팅.',
            result: 'AWS EC2 단일 인스턴스에서 전체 스택 운영 중',
          },
          {
            title: '어드민 블로그 에디터',
            problem: '외부 CMS 없이 직접 글을 작성·관리할 수 있는 인터페이스 필요',
            solution: '커스텀 리치 텍스트 에디터 구현 + Spring Boot REST API 연동. 이미지는 S3 직접 업로드 후 CloudFront URL 삽입.',
            result: '이 사이트의 블로그가 직접 만든 어드민으로 운영 중',
            video: '/mywebsite-editor.webm',
          },
        ],
        features: [
          'GSAP 룸 씬 인트로 애니메이션',
